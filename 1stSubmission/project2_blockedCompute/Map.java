...
public class Map extends Mapper<LongWritable, Text, LongWritable, Text> {
	...
	public void map(LongWritable key, Text nodeInfo, Context context) throws IOException, InterruptedException {
		  ...
		  //System.out.println("************************************* node Id:"+nodeId+" ,current rank: "+pageRank+", its block is: "+blockId);
		  //System.out.println("Replaced myself nodeID: "+nodeId+" NodeInfo:"+nodeInfo.toString());
	...
	static long getBlockID_ofNode(long nodeID)
	{
		if(nodeID<0)
			return -1;
		int blockID = 0;
		long rangeMax = GlobalVals.block[blockID];
		while(nodeID>rangeMax)
		{
			blockID++;
			if(blockID>68)
				return -1;
			rangeMax += GlobalVals.block[blockID];
		}
		//System.out.println("<Mapper> the block of node "+nodeID+" is block:"+blockID);
		return blockID;
...
//			if(mid != 0)
//				beforeMid = GlobalVals.block[mid-1];
//			else
//				beforeMid = 0;
//
//			if( beforeMid < nodeID && nodeID <=GlobalVals.block[mid])
//				return mid;
//			if(GlobalVals.block[mid]<nodeID)
//				left = mid+1;
//			else
//				right = mid-1;
//		}
